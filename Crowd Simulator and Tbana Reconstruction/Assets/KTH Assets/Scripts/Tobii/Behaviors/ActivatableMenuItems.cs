...
public class ActivatableMenuItems : MonoBehaviour
{
    ...
    void Update()
    {
        foreach (Transform menuItem in _menuItems)
        {
            ...
            {
                menuItem.GetComponent<UI.Image>().color = new Color(0.65f, 0.65f, 0.65f, 1);
            }
            else
            {
                menuItem.GetComponent<UI.Image>().color = Color.grey;
            }
        }
    }
    ...
    {
        var image = menuItem.GetComponent<UI.Image>();
        var rectTransform = image.rectTransform;
        var width = rectTransform.sizeDelta.x;
        var height = rectTransform.sizeDelta.y;
        var xPosition = rectTransform.position.x * Screen.width + rectTransform.anchoredPosition.x;
        var yPositionUpperLeft = rectTransform.position.y * Screen.height + rectTransform.anchoredPosition.y + height;
